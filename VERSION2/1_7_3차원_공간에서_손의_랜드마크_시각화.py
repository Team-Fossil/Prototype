...
# 중간 레이어 출력을 얻기 위한 모델 정의
intermediate_layer_model = Model(inputs=model.input,
                                 outputs=model.get_layer(index=-2).output)

# 학습 데이터의 중간 레이어 출력 얻기
intermediate_output = intermediate_layer_model.predict(X_train_expanded)
...
ax.scatter(tsne_result[:, 0], tsne_result[:, 1], tsne_result[:, 2], c=y_train, cmap='viridis', s=60)
